 ...
 */
class KnowledgeAdapter:BaseQuickAdapter<ArticleResponseBody.DatasBean,BaseViewHolder>(R.layout.item_knowledge_list) {
    override fun convert(helper: BaseViewHolder, item: ArticleResponseBody.DatasBean?) {
        ...
        tv_article_chapterName.setText(item.superChapterName+"/"+item.author)
        if(item.collect){
